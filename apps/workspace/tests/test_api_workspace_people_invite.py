...
class TestInviteAPI(APITestCase):
    def setUp(self):
        self.users = {
            ...
            "admin_extra_workspaces": User.objects.create(
        ...
        self.admin_profile = Membership.objects.get(
            object_type=MSP_WORKSPACE, object_uuid=self.workspace.uuid, user=self.users["admin"]
        )
        # make the member_a user member of the workspace
        self.member_profile = Membership.objects.create(
            object_type=MSP_WORKSPACE,
            object_uuid=self.workspace.uuid,
        ...
        self.member_profile2 = Membership.objects.create(
            object_type=MSP_WORKSPACE,
    ...
    def test_cannot_invite_active_member(self):
        ...
        profile = Membership.objects.create(
            object_type=MSP_WORKSPACE,
            object_uuid=self.workspace.uuid,
    ...
    def test_accept_invite(self):
        ...
        profile = Membership.objects.get(pk=self.invitation.pk)
    ...
    def test_check_email_before_invite(self):
        """Check if email already has a membership for the invited object"""
        url = reverse(
            "workspace-people-invite-check-email",
            kwargs={
                "version": "v1",
                "parent_lookup_workspace__suuid": self.workspace.suuid,
            },
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["email_exist"] is None
        ...
        assert response.status_code == status.HTTP_200_OK
        assert response.data["email_exist"] is not None
        assert self.users["user_a"].email in response.data["email_exist"]
