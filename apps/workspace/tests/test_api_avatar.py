...
from account.models.membership import MSP_WORKSPACE, Invitation
from core.tests.base import BaseUserPopulation

pytestmark = pytest.mark.django_db


class BaseAvatarAPI(BaseUserPopulation, APITestCase):
    def setUp(self):
        super().setUp()
...
class TestAvatarUpdateAPI(BaseAvatarAPI):
    def setUp(self):
        super().setUp()
    ...
    def test_update_avatar_as_nonmember(self):
        """
        We can NOT update avatar as nonmember
        """
    ...
    def test_update_avatar_not_on_other_userprofile_as_member(self):
        """
        We cannot update avatars from other people
        """
        token = self.users["member"].auth_token
        self.client.credentials(HTTP_AUTHORIZATION="Token " + token.key)
...
class TestAvatarDeleteAPI(BaseAvatarAPI):
    ...
    def test_delete_avatar_as_member_cannot_delete_otherones(self):
        """
        We can NOT delete avatar if ht profile is not ours
        """
