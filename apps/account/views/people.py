...
from account.serializers.me import AvatarSerializer
...
@extend_schema_view(
    ...
    avatar=extend_schema(description="Update a person's avatar", responses={204: None}),
...
class PeopleViewSet(
    ...
    def initial(self, request, *args, **kwargs):
        ...
        """
        self.object_dict = self.get_parents_query_dict()
        request.data.update(self.object_dict)
    ...
    def invite_resend(self, request, **kwargs):
        ...
        resend_serializer = ResendInviteSerializer(data=request.data)
        resend_serializer.is_valid(raise_exception=True)
        front_end_url = resend_serializer.validated_data.get("front_end_url", settings.ASKANNA_UI_URL)

        invite_serializer = InviteSerializer(instance.invitation)
        invite_serializer.send_invite(front_end_url=front_end_url)
