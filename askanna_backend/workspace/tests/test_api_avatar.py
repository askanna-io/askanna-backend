...
class TestAvatarAPI(APITestCase):
    def setUp(self):
        self.users = {
    ...
    def test_update_avatar_not_on_other_userprofile_as_member(self):
        ...
        payload = {"avatar": self.image_base64}
        response = self.client.patch(self.url_member_b, payload, format="json",)
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
