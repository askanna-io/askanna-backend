...
class RoleUpdateByAdminOnlyPermission(permissions.BasePermission):
    """
    This permission class is to make sure that the role can only be changed by admin users of the workspace. An admin
    can change anyones role except its own role.
...
class RequestHasAccessToMembershipPermission(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        Membership updates can only be done by the membership owner or by admin users of the workspace.
        ...
        if view.action == "destroy":
            ...
            has_permission = (not obj.deleted and obj.user == request.user) or view.request_user_is_workspace_admin
