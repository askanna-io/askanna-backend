...
class BaseAccounts(APITestCase):
    def setUp(self):
        super().setUp()
        self.users = {
            ...
            "user_b": User.objects.create(  # nosec: B106
                username="user_b",
                email="user_b@example.com",
                password="password-user_b",
                name="user b",
...
class BaseWorkspace(BaseAccounts):
    def setUp(self):
        super().setUp()
        signals.post_save.disconnect(install_demo_project_in_workspace, sender=Workspace)
        self.workspace_private = Workspace.objects.create(
            name="test workspace",
            visibility="PRIVATE",
        )
        ...
        self.admin_profile = UserProfile.objects.create(
            ...
            object_uuid=self.workspace_private.uuid,
            user=self.users["workspace_admin"],
            role=WS_ADMIN,
    ...
    def tearDown(self):
        super().tearDown()
        self.workspace_private.delete()
        self.workspace_public.delete()

        self.admin_profile.delete()
        self.member_profile.delete()
        self.member_profile_b.delete()


class BaseProject(BaseWorkspace):
    def setUp(self):
        super().setUp()
        self.project_private = Project.objects.create(
            workspace=self.workspace_private,
            name="test project private",
            description="test private project description",
            visibility="PRIVATE",
        )
        self.project_public = Project.objects.create(
            workspace=self.workspace_public,
            name="test project public",
            description="test public project description",
            visibility="PUBLIC",
        )

    def tearDown(self):
        super().tearDown()
        self.project_private.delete()
        self.project_public.delete()


class BaseAvatar(BaseWorkspace):
    def setUp(self):
        super().setUp()
        self.image_base64_invalid = "data:image/png;base64,c29tZS1pbnZhbGlkLWltYWdlLXN0cmluZw=="
